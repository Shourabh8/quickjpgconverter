...
function compressImage(img, mime, quality) {
  ...
  async function compressFiles() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const blob = await compressImage(img, isLossless ? mime : mime, isLossless ? 1.0 : quality);
      ...
      html += `
        <div class="p-3 rounded-lg border border-border-default bg-surface mb-2">
          ...
          <div class="w-full h-1.5 rounded-full bg-surface-muted overflow-hidden">
            <div class="h-full rounded-full ${item.savings > 0 ? "bg-success" : "bg-brand-500"}" style="width: ${barWidth}%"></div>
          </div>
        </div>
      `;
    });
