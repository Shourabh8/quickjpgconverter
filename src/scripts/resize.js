...
export function initResizer() {
      ...
      html += `
        <div class="flex items-center justify-between p-3 rounded-lg border border-border-default bg-surface mb-2">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="text-xs font-medium text-text-primary truncate">${item.name}</div>
              <div class="text-xs text-text-tertiary">${formatBytes(item.originalSize)} → ${formatBytes(item.blob.size)} · ${item.originalSize > item.blob.size ? "saved " + Math.round((1 - item.blob.size / item.originalSize) * 100) + "%" : "resized"} · ${item.width}×${item.height}</div>
            </div>
          </div>
  ...
  async function downloadAll() {
    const items = resizedBlobs.filter((item) => item && !item.failed);
